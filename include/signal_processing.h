...
struct signal_processing {
    ...
    signal_mask signal_mask; // 信号掩码（Signal Mask）
    signal_handler_flag signal_flags; // 信号处理标志（Signal Processing Flags）
...
namespace signal_processing_set {
     ...
     * 传统的信号处理设置，简单易用但功能有限
    */
    inline signal_handler_t signal(signal_mask sig, signal_handler_t func);
    /**
    ...
    */
    inline int32_t raise(signal_mask sig);
    /**
    ...
    */
    inline int32_t kill(pid_t pid, signal_mask sig);
    /**
    ...
    */
    inline int32_t sigpending(signal_mask& set);
    /**
     * sigpending - 获取当前挂起的信号（Get Pending Signals）
     * @param set 信号掩码引用（Signal Mask Reference）
     * @return 是否成功获取挂起信号,成功返回 0，失败返回非 0（True if Pending Signals Retrieved Successfully, False Otherwise）
    ...
    */
    inline int32_t sigprocmask(sigprocmask_operation how, signal_mask& set, signal_mask* oldset);
    /**
    ...
    */
    inline int32_t sigsuspend(const signal_mask& sigmask);
    /**
    ...
    */
    inline int32_t set_signal_processing(signal_mask sig, const signal_processing& act, signal_processing* oldact);
}